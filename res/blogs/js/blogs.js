...
var mwPostEd = {
	...
	save	: function (dont_hide) {
		...
		if ( cur_cat ) {
			...
			this.Form.find('input[name=filter_by_category_id]').val( cur_cat );
			
		} else {
			
			this.Form.find('input[name=filter_by_category_id]').val( mwData.kind_category_id );
			this.Form.find('input[name=kind_category_id]').val( mwData.kind_category_id );
		...
		var do_field = function(type) {
			...
			if ( type == 'thumb' ) {
				...
				if ( mwData.temp_thumb_name ) {
					...
					if (mwData.temp_thumb_name.indexOf('index.html') == -1  )
						pic_start += 'files/galleries/index.html';
			...
			if ( type == 'banner' ) {
				...
				if ( mwData.temp_banner_name ) {
					...
					//add files/galleries folder in case images selected from gallery just right now
					if (mwData.temp_banner_name.indexOf('index.html') == -1  )
						pic_start += 'files/galleries/index.html';
